    ...
    async with async_playwright() as p:
        ...
        async def click_item(value):
             await page.evaluate(f"""
                  ...
                  let item = items.find(el => el.textContent.trim() === '{value}' || el.getAttribute('data-value') === '{value}');
                  if (item) item.click();
             """)
        ...
        # It's #simple-dynamic-inputs based on grep
        input_container = page.locator("#simple-dynamic-inputs")
        is_visible = await input_container.is_visible()
        print(f"Dynamic Input Container Visible: {is_visible}")

        if is_visible:
            text_area = input_container.locator("textarea")
            count = await text_area.count()
            if count > 0:
                print("Text area successfully converted!")
                placeholder = await text_area.first.get_attribute("placeholder")
                print(f"Placeholder text: {placeholder}")
            else:
                print("No textarea found")
            label_text = await input_container.locator("label").first.inner_text()
            print(f"Label text: {label_text}")

        await page.screenshot(path="verification/web_dev_generate_new_idea_textarea.png", full_page=True)
        print("Screenshot saved to verification/web_dev_generate_new_idea_textarea.png")
